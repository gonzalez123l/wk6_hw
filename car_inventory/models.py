...
class Cars(db.Model):
    ...
    year = db.Column(db.Date, nullable=False, default=datetime.now().date())
    color = db.Column(db.String(100), nullable=False)
    image_url = db.Column(db.String(300), nullable=True)
    user_token = db.Column(db.String, db.ForeignKey('user.token'), nullable=False)


    def __init__(self, model, make, year, color, user_token, image_url=None, id = ''):
        self.id = self.set_id()
        self.model = model
        self.make = make
        self.year = year
...
class CarSchema(ma.Schema):
    class Meta:
        fields = ['id', 'model', 'make','year','color', 'image_url']
